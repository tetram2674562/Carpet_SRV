...
VanillaMinecraftServer::VanillaMinecraftServer()
  : keypair(1024)
  , running(true)
  , server_socket(-1)
  , maxPlayers(20)
...
void*
VanillaMinecraftServer::ServerThread(void* server_ptr)
{
  VanillaMinecraftServer* server =
    static_cast<VanillaMinecraftServer*>(server_ptr);
  ...
  const int bindOutput = bind(
    serverSocket, reinterpret_cast<sockaddr*>(&serverAddr), sizeof(serverAddr));
  if (bindOutput != 0) {
    utils::ConsoleUtils::getInstance().printerr(
      "Failed to bind to port " + utils::ConsoleUtils::toString(PORT));
    return 0;
  }
  listen(serverSocket, 5);

  utils::ConsoleUtils::getInstance().printMessage(
    "Server listening on port " + utils::ConsoleUtils::toString(PORT) + "...");
  ...
  for (size_t i = 0; i < this->players.size(); i++) {
    if (this->players[i] != NULL) {
      this->players[i]->kickPlayer(
        utils::ConsoleUtils::createUTF16String("Server is restarting..."));
...
VanillaMinecraftServer::requestKickPlayer(const entity::Player* player,
                                          const utils::UTF16String& reason)
{
 ...
 */
void
VanillaMinecraftServer::handleClient(const int clientSocket)
{
 ...
 */
int
VanillaMinecraftServer::getServerSocket() const
{
  return this->server_socket;
}

void
VanillaMinecraftServer::setServerSocket(const int serverSocket)
{
