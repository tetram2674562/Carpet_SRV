...
KeyPair::KeyPair(int bits) : keypair(nullptr) {

  EVP_PKEY_CTX *ctx = EVP_PKEY_CTX_new_id(EVP_PKEY_RSA, nullptr);
  check(ctx != nullptr, "Failed to create EVP_PKEY_CTX");
  check(EVP_PKEY_keygen_init(ctx) > 0, "Failed to initialize keygen");
  check(EVP_PKEY_CTX_set_rsa_keygen_bits(ctx, bits) > 0,
...
vector<unsigned char> KeyPair::getPrivateKeychars() const {
  ...
  check(i2d_PrivateKey(keypair, &ptr) > 0,
        "Could not write private key to DER");

  return out;
}

std::vector<unsigned char> KeyPair::getPublicKeychars() const {
  const int len = i2d_PUBKEY(keypair, nullptr);
...
std::string KeyPair::getPrivateKeyPEM() const {
  ...
  check(PEM_write_bio_PrivateKey(bio, keypair, nullptr, nullptr, 0, nullptr,
                                 nullptr),
...
std::vector<unsigned char> KeyPair::decryptWithPrivateKey(
    const std::vector<unsigned char> &encrypted) const {
  ...
  if (EVP_PKEY_decrypt(ctx, decrypted.data(), &outlen, encrypted.data(),
                       encrypted.size()) <= 0) {
    EVP_PKEY_CTX_free(ctx);
