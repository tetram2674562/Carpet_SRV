...
static void
check(bool success, const char* message)
{
  if (!success)
    throw runtime_error(message);
}

KeyPair::KeyPair(int bits)
  : keypair(NULL)
{
...
KeyPair::getPrivateKeychars() const
{
...
KeyPair::getPublicKeychars() const
{
...
std::string
KeyPair::getPrivateKeyPEM() const
{
  ...
  const std::vector<unsigned char>& encrypted) const
{
  ...
  if (EVP_PKEY_decrypt(
        ctx, decrypted.data(), &outlen, encrypted.data(), encrypted.size()) <=
      0) {
    EVP_PKEY_CTX_free(ctx);
