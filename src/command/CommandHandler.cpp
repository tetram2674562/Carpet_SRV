...
void*
CommandHandler::CommandListener(void* instance)
{
  std::string command;
  ...
  while (self->isRunning()) {
    ...
    if (command == "stop") {
        ...
        for (int i = 0; i < nbPlayer; ++i) {
          ...
          if (player) {
            server::VanillaMinecraftServer::getServer().requestKickPlayer(
              player,
              utils::ConsoleUtils::createUTF16String(
                "Kicked from the server by the console."));
          }
        }
        utils::ConsoleUtils::getInstance().printMessage(
          "Kicked " + utils::ConsoleUtils::toString(nbPlayer) +
      ...
      if (nbPlayer == 0) {
        ...
        utils::ConsoleUtils::getInstance().printMessage(
          "There are " + utils::ConsoleUtils::toString(nbPlayer) +
