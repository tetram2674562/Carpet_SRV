...
class Test {
public:
  Test(const char *name, const char *test_case_name)
      : m_name(name), m_status(None), m_test_case_name(test_case_name) {}
  virtual ~Test() {}
  const std::string &name() const { return m_name; }
  const std::string &case_name() const { return m_test_case_name; }
  TestStatus status() const { return m_status; }
  virtual void run() {
    std::cout << "[ RUN      ] " << m_test_case_name << "." << m_name
              << std::endl;
    m_status = Success;
    run_internal(m_status);
    if (m_status == Success) {
      std::cout << "[       OK ] " << m_test_case_name << "." << m_name
                << std::endl;
    } else {
      std::cout << "[  FAILED  ] " << m_test_case_name << "." << m_name
                << std::endl;
    }
  }
...
public:
  ...
  void run() {
    std::cout << "[----------] " << m_tests.size() << " tests from " << name()
              << std::endl;
    for (size_t i = 0; i < m_tests.size(); ++i)
      m_tests[i]->run();
    std::cout << "[----------] " << m_tests.size() << " tests from " << name()
  ...
  void print_failed() {
    for (size_t i = 0; i < m_tests.size(); ++i)
      if (m_tests[i]->status() == Failed)
        std::cout << "[  FAILED  ] " << m_name << "." << m_tests[i]->name()
                  << std::endl;
  }
...
inline TestCase *find_test_case(const std::string &name,
...
public:
  ...
  int run_tests() {
    std::cout << "[==========] Running " << m_tests.size() << " tests from "
              << m_test_cases.size() << " test cases.\n";

    for (size_t i = 0; i < m_test_cases.size(); ++i)
      m_test_cases[i]->run();

    std::cout << "[==========] " << m_tests.size() << " tests from "
              << m_test_cases.size() << " test cases ran.\n";
...
#define EXPECT_THROW(what, exception_type)                                     \
  do {                                                                         \
    ...
    try {                                                                      \
      ...
      ftest_caught_it = FTEST_RIGHT_TYPE;                                      \
    } catch (...) {                                                            \
      ftest_caught_it = FTEST_WRONG_TYPE;                                      \
    }                                                                          \
    if (ftest_caught_it == FTEST_NONE) {                                       \
      std::cout << __FILE__ << ":" << __LINE__ << ": Failure\n";               \
      std::cout << "  Expected : " #what                                       \
...
#define TEST(f_test_case_name, f_test_name)                                    \
  ...
  public:                                                                      \
    F_TEST_##f_test_case_name##f_test_name(const char *test_name,              \
  ...
  bool F_TEST_##f_test_case_name##f_test_name::m_registered =                  \
      ftest::testdriver.add_test(new F_TEST_##f_test_case_name##f_test_name(   \
          #f_test_name, #f_test_case_name));                                   \
  void F_TEST_##f_test_case_name##f_test_name::run_internal(                   \
      ftest::TestStatus &status)
